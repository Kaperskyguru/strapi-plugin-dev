...
export default ({ strapi }: { strapi: Strapi }) => ({
  async all() {
    const results =
      (await strapi.query("plugin::prettify.prettify-link").findMany()) || [];
    return results;
  },
  ...
  async create(data: ILink) {
    ...
    const results = await strapi
      .query("plugin::prettify.prettify-link")
      .create({ data });
  ...
  async find(params) {
    const link = await strapi.query("plugin::prettify.prettify-link").findOne({
      where: { id: params.id },
    });
...
const getPrettyLink = async (code: string) => {
  const setting = await strapi
    .query("plugin::prettify.prettify-setting")
    .findOne({
...
const findDuplicateURLCode = async (
  ...
  try {
    const results: ILink[] =
      (await strapi.entityService.findMany("plugin::prettify.prettify-link", {
        filters: {
          link: code,
